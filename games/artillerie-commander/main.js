...
let players = [], terrain = terrainFor(Date.now()), particles = [], trails = [], effects = [], collapses = [];
...
function addEffect(kind,x,y,radius,delay=0,extra={}){effects.push({kind,x,y,radius,age:-delay,duration:extra.duration??(kind==='acid'?1.1:kind==='nuke'?0.72:0.58),...extra});}
function drawEffects(){for(const e of effects){if(e.age<0)continue;const t=clamp(e.age/e.duration,0,1),grow=e.kind==='dirt'?Math.min(1,t*2.2):.25+t*.75,r=e.radius*grow,ey=e.targetY===undefined?e.y:e.y+(e.targetY-e.y)*Math.max(0,(t-.25)/.75)**2;ctx.save();if(e.kind==='nuke'&&t<.45){ctx.globalCompositeOperation='difference';ctx.fillStyle=t%0.14<.07?'#fff':'#8ff';ctx.beginPath();ctx.arc(e.x,ey,r,0,Math.PI*2);ctx.fill();}else if(e.kind==='dirt'){ctx.globalAlpha=1-t*.15;ctx.fillStyle='#9b6a3c';ctx.strokeStyle='#d4ef72';ctx.lineWidth=3;ctx.beginPath();ctx.arc(e.x,ey,r,0,Math.PI*2);ctx.fill();ctx.stroke();}else if(e.kind==='acid'){ctx.globalAlpha=1-t;ctx.strokeStyle='#b9ff4c';ctx.lineWidth=3+6*(1-t);for(let i=-2;i<=2;i+=1){ctx.beginPath();ctx.moveTo(e.x+i*e.radius*.18,ey-e.radius*.45);ctx.lineTo(e.x+i*e.radius*.13,ey+e.radius*(.2+t*1.7));ctx.stroke();}}else if(e.kind==='drill'){ctx.globalAlpha=1-t;ctx.strokeStyle='#fff4b0';ctx.lineWidth=4;ctx.beginPath();for(let i=0;i<8;i+=1)ctx.lineTo(e.x+Math.sin(i*2.4+t*12)*e.radius*.45,ey+i*e.radius*.34);ctx.stroke();}else{ctx.globalAlpha=1-t;ctx.strokeStyle=e.kind==='heavy'?'#fff1a0':'#ffb13b';ctx.lineWidth=e.kind==='heavy'?8:5;ctx.beginPath();ctx.arc(e.x,ey,r,0,Math.PI*2);ctx.stroke();if(e.kind==='heavy'||e.kind==='mirv'){ctx.lineWidth=2;ctx.beginPath();ctx.arc(e.x,ey,r*.58,0,Math.PI*2);ctx.stroke();}}ctx.restore();}}
function drawCollapses(){ctx.fillStyle='#8b5938';for(const collapse of collapses){const t=clamp(collapse.age/collapse.duration,0,1);for(const [x,c] of collapse.columns)if(x%5===0){const y=c.original+(c.cut-c.original)*t;ctx.fillRect(x-2,y,5,Math.max(3,c.original+c.loss-y+6));}}}
function render(){screenBackdrop();worldTransform();drawBackdrop();drawTerrain();
  ...
  drawEffects();drawCollapses();ctx.fillStyle='#ffffff55';for(const t of trails)ctx.fillRect(t.x-1,t.y-1,2,2);for(const p of particles){ctx.globalAlpha=p.life;ctx.fillStyle=p.color;ctx.beginPath();ctx.arc(p.x,p.y,p.size*p.life,0,Math.PI*2);ctx.fill();}ctx.globalAlpha=1;requestAnimationFrame(render);}
...
function startLaser(p,w,energy){const a=p.angle*Math.PI/180,muzzle=muzzlePoint(p);laserState={shooter:p,weapon:w,energy,x:muzzle.x,y:muzzle.y,dx:Math.cos(a)*p.facing,dy:-Math.sin(a),points:[muzzle],distance:0,killed:[],cuts:new Map()};trails=[];}
function beginCollapse(columns,done){if(!columns?.size){done();return;}collapses.push({columns,age:0,duration:.62,done});}
function applyImpact(x,y,weapon,shooter,directTarget=null){const aliveBefore=new Set(living(players));let cuts=null;if(directTarget&&directTarget.hp>0){const bonus=Math.round(weapon.damage*.55),wasAlive=directTarget.hp>0;directTarget.hp=Math.max(0,directTarget.hp-bonus);if(directTarget!==shooter){shooter.score+=bonus*2;if(wasAlive&&directTarget.hp===0)shooter.score+=100;}}if(weapon.acid){cuts=new Map();const start=Math.max(0,Math.floor(x-weapon.radius)),end=Math.min(terrain.length-1,Math.ceil(x+weapon.radius));for(let ix=start;ix<=end;ix+=1){const falloff=Math.sqrt(Math.max(0,1-((ix-x)/weapon.radius)**2));if(Math.random()>falloff*.82)continue;const original=terrain[ix],loss=Math.max(2,Math.round((3+Math.random()*weapon.radius*.42)*falloff)),cut=Math.min(WORLD.height,original+loss+Math.round(Math.random()*weapon.radius*.9));terrain[ix]=cut;cuts.set(ix,{original,cut,loss});}damageAt(players,x,y,weapon,shooter);}else{const count=weapon.cluster||1,bursts=Array.from({length:count},(_,i)=>(i-(count-1)/2)*24);for(const offset of bursts){const bx=clamp(x+offset,0,WORLD.width-1);crater(terrain,bx,y,weapon.radius,weapon.dirt);damageAt(players,bx,y,weapon,shooter);for(let i=0;i<Math.min(90,weapon.radius*1.2);i+=1)particles.push({x:bx,y,vx:(Math.random()-.5)*240,vy:-Math.random()*210,life:1,size:3+Math.random()*7,color:weapon.dirt?'#a88a55':i%3?'#ff9f36':'#fff2a8'});}}if(weapon.burrow)for(let i=1;i<5;i+=1)crater(terrain,x,y+i*17,weapon.radius*.72);return {deaths:players.filter((p)=>aliveBefore.has(p)&&p.hp<=0&&!p.deathTriggered),cuts};}
function completeImpact(deaths){settleTanks();if(deathSalvo&&deaths.length){deaths.forEach((p,i)=>setTimeout(()=>triggerDeathSalvo(p),i*520));setTimeout(finishTurn,deaths.length*520+900);}else setTimeout(finishTurn,650);}
function explode(x,y,weapon,shooter,directTarget=null){projectile=null;focusOn(x,y);let impactY=y,delay=weapon.acid?720:weapon.effect==='nuke'?300:140;if(weapon.dirt){const ground=surfaceAt(terrain,x),distance=Math.max(0,ground-y),duration=.58+Math.min(1.1,distance/360);impactY=ground;delay=duration*1000;addEffect(weapon.effect,x,y,weapon.radius,0,{duration,targetY:ground-weapon.radius*.72});}else if(weapon.cluster){for(let i=0;i<weapon.cluster;i+=1)addEffect(weapon.effect,x+(i-(weapon.cluster-1)/2)*24,y,weapon.radius,i*.07);}else addEffect(weapon.effect,x,y,weapon.radius);setTimeout(()=>{const result=applyImpact(x,impactY,weapon,shooter,directTarget);if(result.cuts)beginCollapse(result.cuts,()=>completeImpact(result.deaths));else completeImpact(result.deaths);},delay);}
function triggerDeathSalvo(p){p.deathTriggered=true;const pool=WEAPONS.filter((w)=>w.id!=='granate'&&!w.laser),weapon=pool[Math.floor(Math.random()*pool.length)];addEffect(weapon.effect,p.x,p.y,weapon.radius);setTimeout(()=>{const result=applyImpact(p.x,p.y,weapon,p);if(result.cuts)beginCollapse(result.cuts,()=>settleTanks());else settleTanks();},weapon.acid?700:weapon.dirt?380:160);}
...
function finishLaser(){if(!laserState)return;const {shooter,killed,cuts}=laserState;shooter.lastTrail=[...laserState.points];laserState=null;beginCollapse(cuts,()=>{settleTanks();if(deathSalvo&&killed.length){killed.forEach((p,i)=>setTimeout(()=>triggerDeathSalvo(p),i*520));setTimeout(finishTurn,killed.length*520+900);}else setTimeout(finishTurn,500);});}
function stepLaser(dt){const laser=laserState;if(!laser)return;let travel=Math.min(1000*dt,laser.energy*2),blocked=false;while(travel>0&&!blocked){const step=Math.min(4,travel),nx=laser.x+laser.dx*step,ny=laser.y+laser.dy*step,hit=tankAt(players,nx,ny,laser.shooter,laser.distance/1000);if(hit){const damage=Math.min(hit.hp,72*dt);hit.hp=Math.max(0,hit.hp-damage);if(hit!==laser.shooter){laser.shooter.score+=Math.round(damage*2);if(hit.hp===0&&!laser.killed.includes(hit)){laser.shooter.score+=100;laser.killed.push(hit);}}laser.energy-=500*dt;blocked=hit.hp>0;if(!blocked)continue;break;}const probe={x:nx,y:ny,vx:laser.dx,vy:laser.dy};const edge=boundaryHit(probe,wallMode);if(edge&&edge.action!=='bounce'){laser.energy=0;break;}if(edge?.action==='bounce'){laser.dx=probe.vx;laser.dy=probe.vy;}laser.x=probe.x;laser.y=probe.y;laser.distance+=step;laser.energy-=step/2;travel-=step;laser.points.push({x:laser.x,y:laser.y});const tx=Math.round(laser.x);if(laser.y>=surfaceAt(terrain,tx))for(let ix=tx-5;ix<=tx+5;ix+=1)if(ix>=0&&ix<terrain.length){const previous=laser.cuts.get(ix),original=previous?.original??terrain[ix],loss=(previous?.loss??0)+2,cut=Math.max(previous?.cut??original,Math.round(laser.y+7));laser.cuts.set(ix,{original,cut,loss});terrain[ix]=cut;}}if(!camera.manual)focusOn(laser.x,laser.y);if(laser.energy<=0)finishLaser();}
function tick(now){const dt=Math.min(.025,(now-(tick.last||now))/1000);tick.last=now;if(projectile){stepProjectile(projectile,dt,wind);trails.push({x:projectile.x,y:projectile.y});if(trails.length>400)trails.shift();projectile.shooter.lastTrail=[...trails];if(!camera.manual)focusOn(projectile.x,projectile.y);
  ...
  if(laserState)stepLaser(dt);for(const e of effects)e.age+=dt;effects=effects.filter((e)=>e.age<e.duration);const completed=[];for(const c of collapses){c.age+=dt;if(c.age>=c.duration){for(const [x,column] of c.columns)terrain[x]=Math.min(WORLD.height,column.original+column.loss);completed.push(c);}}collapses=collapses.filter((c)=>!completed.includes(c));for(const c of completed)c.done();for(const p of particles){p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=260*dt;p.life-=dt*1.25;}particles=particles.filter((p)=>p.life>0);requestAnimationFrame(tick);}
